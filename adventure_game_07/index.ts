...
do{
   if (opponentplayer.select === "Start"){
      ...
      let ask:any = await inquirer.prompt({
        type:    "list",
        name:    "opt",
       ...
       if(ask.opt === "attack"){
           ...
           if(num > 0)  {
              ...
              if (P1.fuel<= 0){console.log(chalk.bold.redBright("You loose, better luck next time"))};
            };
           if(num <= 0)  { opponent1.fuelDecrease()
              console.log(chalk.bold.red(`${opponent1.name} fuel  is ${opponent1.fuel}`))
              console.log(chalk.bold.green(`${P1.name} fuel  is ${P1.fuel}`))
                    if (P1.fuel<= 0){console.log(chalk.bold.greenBright("You Win"))};         
            }; 
        };
